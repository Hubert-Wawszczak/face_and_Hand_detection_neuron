...
def preprocess(face_image):
    # Modify this function to match the preprocessing required by your model
    processed_face = cv2.resize(face_image, (224, 224))  # Example: Resize to model's input size
...
def face_detection_process(frame_queue, face_queue, log_queue, running, model):
    ...
    while running.value:
        ...
        for face in faces:
            ...
            processed_face = preprocess(cropped_face)
            prediction = model.predict(processed_face)
            recognized_face_label = np.argmax(prediction, axis=1)  # Assuming class indices output
            recognized_faces.append((recognized_face_label, face['box']))
        elapsed_time = time.time() - start_time
        log_queue.put(f"Face detection and recognition time: {elapsed_time:.3f} seconds")
        face_queue.put(recognized_faces)
...
def main():
    ...
    model = tf.keras.models.load_model(r'D:\Repo\Python\faacedetct\save\fine_tuning.h5')
    ...
    face_process = Process(target=face_detection_process, args=(frame_queue, face_queue, log_queue, running, model))
    ...
    try:
        while cap.isOpened():
            ...
            # Drawing last known face recognitions
            for recognized_label, box in last_faces:
                x, y, w, h = box
                cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 155, 255), 2)
                label_text = f"Person {recognized_label[0]}"  # Modify as per your model's output
